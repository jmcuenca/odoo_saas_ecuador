...
class PosOrder(models.Model):
    ...
    l10n_ec_sri_access_key = fields.Char(string="SRI Access Key", copy=False, help="49-digit Access Key")
    ...
    def _generate_pos_access_key(self):
        ...
        for order in self:
            if not order.l10n_ec_sri_access_key:
                ...
                env = '2' if order.company_id.l10n_ec_sri_environment == 'production' else '1'
                ...
                # Extract digits
                seq_num = ''.join(filter(str.isdigit, seq_raw))[-9:] or str(order.id).zfill(9)
