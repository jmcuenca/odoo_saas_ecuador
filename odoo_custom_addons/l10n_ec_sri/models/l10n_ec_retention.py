...
class L10nEcRetention(models.Model):
    ...
    invoice_id = fields.Many2one('account.move', string='Invoice', domain="[('move_type', '=', 'in_invoice')]", required=True)
    partner_id = fields.Many2one('res.partner', related='invoice_id.partner_id', string='Supplier', store=True)
    date_issue = fields.Date(string='Date Issue', default=fields.Date.context_today, required=True)

    # Tax Lines
    tax_ids = fields.One2many('l10n_ec.retention.line', 'retention_id', string='Withholding Lines')
    ...
    l10n_ec_sri_status = fields.Selection([
        ...
        ('rejected', 'Rejected'),
    ], string='SRI Status', default='draft', copy=False)
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            ...
            vals['name'] = self.env['ir.sequence'].next_by_code('l10n_ec.retention') or _('New')
    ...
    def action_send_sri(self):
        ...
        for ret in self:
            ...
            response = self.env['l10n_ec.sri.service'].send_document(
                signed_xml,
                environment=ret.company_id.l10n_ec_sri_environment
            )
    ...
    def action_check_sri(self):
        ...
        for ret in self:
            ...
            response = self.env['l10n_ec.sri.service'].check_authorization(ret.l10n_ec_sri_access_key)
...
class L10nEcRetentionLine(models.Model):
    ...
    retention_id = fields.Many2one('l10n_ec.retention', string='Retention', ondelete='cascade')
    ...
    currency_id = fields.Many2one('res.currency', related='retention_id.company_id.currency_id')
