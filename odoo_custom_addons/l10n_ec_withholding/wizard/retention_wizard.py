...
class RetentionWizard(models.TransientModel):
    ...
    line_ids = fields.One2many('l10n_ec.retention.wizard.line', 'wizard_id', string='Withholding Lines')

    @api.model
    def default_get(self, fields_list):
        res = super(RetentionWizard, self).default_get(fields_list)
        if 'active_id' in self.env.context and self.env.context.get('active_model') == 'account.move':
            ...
            total_vat = sum(line.price_total - line.price_subtotal for line in invoice.invoice_line_ids)
            if total_vat > 0:
                lines.append((0, 0, {
                    'tax_type': '2', # IVA
    ...
    def action_create_retention(self):
        ...
        for line in self.line_ids:
            retention_vals['retention_line_ids'].append((0, 0, {
