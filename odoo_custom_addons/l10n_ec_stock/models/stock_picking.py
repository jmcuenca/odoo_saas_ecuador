...
class StockPicking(models.Model):
    ...
    l10n_ec_transport_reason = fields.Selection([
        ...
        ('otros', 'Otros')
    ], string="Reason for Transport", default='ventas', copy=False)

    l10n_ec_start_date = fields.Date(string="Start Date", default=fields.Date.context_today)
    ...
    def _generate_access_key(self):
        ...
        """
        for record in self:
             if not record.l10n_ec_sri_access_key and record.l10n_ec_driver_id: # Only generate if driver assigned (proxy for ready)
                ...
                key = AccessKey.generate(
                    invoice_date=record.scheduled_date.date() if record.scheduled_date else fields.Date.today(),
                    doc_type='06',
    ...
    def action_send_guia_sri(self):
        ...
        for record in self:
            if not record.l10n_ec_driver_id or not record.l10n_ec_vehicle_id:
                raise UserError(_("Driver and Vehicle are required for SRI Transmission."))
            ...
            xml_content = self.env['ir.qweb']._render('l10n_ec_stock.l10n_ec_guia_xml', values)
