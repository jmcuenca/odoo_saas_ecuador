...
class L10nEcAtsWizard(models.TransientModel):
    ...
    def action_print_form103(self):
        ...
        for p in data['purchases']:
            for r in p.get('retentions', []):
                ...
                if code not in retention_summary:
                    retention_summary[code] = {'code': code, 'name': 'Retention ' + code, 'base': 0.0, 'percent': r['percent'], 'amount': 0.0}
        ...
        return self.env.ref('l10n_ec_reports.action_report_form_103').report_action(self, data={'data': report_data})
    ...
    def action_print_form104(self):
        ...
        purchases_base = sum(p['baseImponible'] + p['baseImpGrav'] + p['baseNoGraIva'] for p in data['purchases'])
        ...
        return self.env.ref('l10n_ec_reports.action_report_form_104').report_action(self, data={'data': report_data})
    ...
    def generate_xml(self):
        ...
        values = {
            ...
            'total_sales': sum(s['baseImpGrav'] + s['baseNoGraIva'] for s in data['sales']),
    ...
    def _get_ats_data(self):
        ...
        for inv in in_invoices:
            # Map ID Type
            if inv.partner_id.l10n_ec_identifier_type == 'ruc': tpId = '01'
            elif inv.partner_id.l10n_ec_identifier_type == 'cedula': tpId = '02'
            else: tpId = '03'
            ...
            for line in inv.line_ids:
                if line.tax_line_id:
                    if hasattr(line.tax_line_id, 'l10n_ec_ice_category_id') and line.tax_line_id.l10n_ec_ice_category_id:
                        monto_ice += abs(line.balance)
                    elif line.tax_group_id.name == 'ICE':
                         monto_ice += abs(line.balance)
                    elif 'IVA' in line.tax_group_id.name or 'VAT' in line.tax_group_id.name:
            ...
            purchases_data.append({
                ...
                'estab': estab, 'ptoEmi': pto, 'secuencial': sec,
        ...
        for key, val in sales_agg.items():
            partner = val['partner']
            if partner.l10n_ec_identifier_type == 'ruc': tpId = '04'
            elif partner.l10n_ec_identifier_type == 'cedula': tpId = '05'
            else: tpId = '06'
